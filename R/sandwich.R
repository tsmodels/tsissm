#' Score Method
#'
#' @param x an object of class \dQuote{tsissm.estimate}.
#' @param ... not currently used.
#' @return The score matrix
#' @details The function returns the scores of the negative of the log likelihood
#' at the optimal solution.
#' @method estfun tsissm.estimate
#' @aliases estfun.tsissm.estimate
#' @rdname estfun
#' @author Alexios Galanos
#' @export
#'
estfun.tsissm.estimate <- function(x, ...)
{
    s <- value(x$score_promise$promise)
    s <- s[which(x$spec$good == 1), , drop = FALSE]
    colnames(s) <- x$parmatrix[estimate == 1]$parameters
    return(s)
}
...
#' @method bread tsissm.estimate
#' @aliases bread.tsissm.estimate
#' @rdname bread
#' @author Alexios Galanos
#' @export
#'
bread.tsissm.estimate <- function(x, ...)
{
    H <- x$hessian
    H <- solve(H)
    return(H)
...
meat_tsissm <- function(x, adjust = FALSE, ...)
{
    psi <- estfun(x, ...)
    k <- NCOL(psi)
    n <- NROW(psi)
...
meatHAC_tsissm <- function(x, prewhite = FALSE, weights = NULL,  lag = NULL,
